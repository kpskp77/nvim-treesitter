...
local function run_install(cache_folder, install_folder, lang, repo, with_sync, generate_from_grammar)
  ...
  ---@type Command[]
  local command_list = {}
  if not from_local_path and not configs.get_install_offline() then
  ...
  if not from_local_path and not configs.get_keep_cache() then
    vim.list_extend(command_list, { shell.select_install_rm_cmd(cache_folder, project_name) })
  end
...
local function install(options)
  ...
  return function(...)
    ...
    local cache_folder = configs.get_cache_dir()
    assert(cache_folder)
